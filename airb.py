...
def getAmenitiesPage(detailpage):
    ...
    driver = setupDriver("https://airbnb.com" + link,
                         5)  # Amenitiespage is a link disguished as a button, this is why I need to do this
    html = driver.page_source
    driver.close()
    return BeautifulSoup(html, features="lxml")
...
def getAmenities(amenitiespage):
    ...
    for amenity in amenities:
        output += re.findall('[A-Z][^A-Z]*', amenity.text)[
                      0] + "**-**"  # **-** will be used to split the string later for the purpose of dummification
    return output
...
def cleanRoomInfo(df):
    ...
    df.loc[:, "beds"] = df.loc[:, "roominfo"].str.split(" . ", n=0, expand=True)[2].str.replace(" bed", "").str.replace(
        "s", "")
...
def cleanPrice(df):
    df.loc[:, "pricepernight"] = \
    df.loc[:, "price"].str.split("Discounted", n=0, expand=True)[0].str.replace("$", "/").str.split("/", n=0,
                                                                                                    expand=True)[1]
    df.loc[:, 'discountedpricepernight'] = \
    df.loc[:, "price"].str.split("Discounted", n=0, expand=True)[1].str.replace("$", "/").str.split("/", n=0,
                                                                                                    expand=True)[1]
    df.loc[:, "price"] = pd.to_numeric(df.pricepernight.str.replace(",", "").str.strip())
    df.loc[:, "discountedprice"] = pd.to_numeric(df.discountedpricepernight.str.replace(" ", "").str.replace(",", ""),
                                                 errors="coerce")
...
def cleanAmenities(df):
    ...
    df.loc[:, "amenities"] = df.amenities.str.replace(" ", "_").str.replace("-", " ").str.replace("*",
                                                                                                  "")  # split in two because of a python bug (https://stackoverflow.com/questions/3675144/regex-error-nothing-to-repeat)
    vectorizer = CountVectorizer(decode_error="ignore")
...
def scraper(urls: object, sample_size: object = None, random_state: object = 1234) -> object:
    df = scrapeURLs(urls)
    df = clean(df)
...
#df2 = scraper(urls2, sample_size = 10).text
print df2
